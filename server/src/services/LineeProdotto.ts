...
var defaults: any = _.partialRight(_.assignWith, customizer);

export async function GetLineeProdotto(elemento: any) {


    return new Promise(async (resolve, reject) => {
        const sage = await SageDb.connect();
        ...
        const values = await sage.query(
            `select CFGFLDNUM1_0,CFGFLDNUM2_0,CFGFLDNUM3_0,CFGFLDNUM4_0,CFGFLDNUM5_0,CFGFLDNUM6_0,CFGFLDALP1_0,CFGFLDALP2_0,CFGFLDALP3_0,CFGFLDALP4_0,CFGFLDALP5_0,CFGFLDALP6_0 from PRODEKD.ITMMASTER where ITMREF_0='${elemento}'`
        );
...
export async function GetStatistico1(elemento: any) {
    const sage = await SageDb.connect();
...
export async function GetStatistico2(elemento: any) {
    const sage = await SageDb.connect();
